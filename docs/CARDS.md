...
- **Blink and spare the thrifts** `us-blink` (SERVE · soft)
  ...
  Volcker has the power over interest rates. You blinked. Your name is still on the door. The hike still happens. You serve the chair's fear of the savings-and-loan lobby. History still rides.
...
The government is about to insure each deposit up to $410,000 (was $100,000 then, adjusted to 2026 CPI) instead of $160,000 (was $40,000 then, adjusted to 2026 CPI). A broker can now gather money from around the country and park it with you, because every dollar is insured. People call those brokered deposits. Your old cheap mortgages still do not pay enough. You can buy this new money and try to grow out of the hole.
...
It is still Carter's year. Savings and loans say they will die unless you raise the government insurance on each deposit from $160,000 (was $40,000 then, adjusted to 2026 CPI) to $410,000 (was $100,000 then, adjusted to 2026 CPI). Deposit insurance is a promise: if the bank fails, Washington pays the customer. Make that promise bigger and a dying bank can attract more money to gamble with.
...
- **Take the seizure** `ir-seized` (HISTORY · walk)
  Lincoln dies. The RTC sells the dirt. Your name is off the door.
...
- **Stay a resolution man** `ir-stay-resolution` (HINDSIGHT · walk)
  ...
  Overlay title: Yes. That was the right thing to do.
  You stayed with the residue. Greenwich still built the model.
...
- **Keep a standing resolution desk** `us-keep-rtc` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  You kept a desk that hunts vehicles. Greenwich still got a weekend.
...
- **Be everything** `ir-glba-yes` (HISTORY · hard)
  Take deposits and run a trading floor. Commercial plus investment. One firm.
...
- **Split the auditor** `ir-split-andersen` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  Andersen only signed. Someone still had to drive to the SPE. You did not.
...
Arthur Levitt wants to split the accountant who signs the books from the accountant who sells advice. Kenneth Lay's letter is on the desk. Enron is a friend of the building. Andersen is both auditor and consultant to Enron. That is the whole fight.

- **Water Levitt down** `us-water-levitt` (HISTORY · soft)
  Independence can wait. Enron wrote a letter. The current setup is cheaper.
- **Back Levitt** `us-back-levitt` (HINDSIGHT · hard)
  The accountant who signs cannot also be the consultant.
  Overlay title: Yes. That was the right thing to do.
  You backed Levitt. Enron still had SPEs. Someone still had to drive to the address.
...
- **Watch the bankruptcy** `us-watch-enron` (HISTORY · ignore)
  The filing will handle it. Enron is a campaign friend. The audit passed.
...
Enron. A campaign. An energy book. The filing is clean until it is not. Nobody has driven to the SPE, the paper company sitting off the books.
...
- **Make them keep the loan** `us-underwrite` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  Originators kept the risk. The AAA factory slowed. The country still wanted the dream.
...
You are the Street after Enron. The old haircut on trades is about to become your own math. The houses are already a factory. Forty dollars of bets on every dollar of yours is a costume Sarbanes-Oxley does not see. They will say Europe will take the holding company if you stay at the old rule.
...
- **Stay at twelve to one** `ir-cse-no` (HINDSIGHT · walk)
  ...
  Overlay title: Yes. That was the right thing to do.
  You kept the haircut. The other four did not.
...
- **Keep twelve to one** `us-keep-12` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  The haircut stayed. The houses still wanted to be AAA.
...
- **Stop at the first bond** `ir-stop-slices` (HINDSIGHT · walk)
  ...
  Overlay title: Yes. That was the right thing to do.
  You stopped at the first bond. The leftover stayed leftover. The factory next door sliced it anyway.
...
- **Let them slice** `us-let-slices` (HISTORY · ignore)
  A bond of bonds is a private product. People want safe paper. The leftover is the next guy's problem.
- **Make them keep the leftover** `us-keep-leftover` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  The leftover stayed leftover. The factory still wanted a second pile.
...
- **Slow the pipeline** `ir-listen-rajan` (HINDSIGHT · soft)
  ...
  Overlay title: Yes. That was the right thing to do.
  You listened at Jackson Hole. The factory next door did not.
...
- **Listen to Rajan** `us-listen-rajan` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  You listened. The houses were still a factory.
...
- **Stop originating** `ir-leave-table` (HINDSIGHT · walk)
  ...
  Overlay title: Yes. That was the right thing to do.
  You left the table. The factory next door kept talking. The shorts still doubled.
...
- **Ask what the shorts heard** `us-ask-dinner` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  You asked what the shorts heard. August still said contained.
...
- **Sell to JPMorgan** `ir-bear-sold` (HISTORY · deal)
  A weekend price. The name on the door dies. The window backstops the buyer.
...
You are Richard Fuld. The room of fourteen banks is not coming. 1998 was a put for Greenwich, a backstop we called private. Enron went to jail. You are the country now. There is no warehouse that closes this.
...
- **Take the discount window** `ir-aig-window` (HISTORY · deal)
  Washington takes 79.9 percent. You live. The name on the door is no longer yours.
...
If AIG Financial Products fails, every desk that bought the insurance is naked. This is Long-Term Capital Management with a country attached.
...
- **Raise capital without them** `ir-raise-capital` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  You raised without TARP. The rest of the street waited.
...
- **Whip it through the first time** `us-tarp-yes-1` (HINDSIGHT · deal)
  ...
  Overlay title: Yes. That was the right thing to do.
  You whipped TARP through the first time. The ticker still hated you.
...
- **Pause the book for the vote** `ir-pause-deal` (HINDSIGHT · soft)
  ...
  Overlay title: Yes. That was the right thing to do.
  You paused origination while they voted. The rest of the street did not.
...
They will make you take the money. The name on the door stays. The real power moved to the discount window. You will say you did not need it.
...
- **Refuse the capital** `ir-refuse-tarp` (SERVE · hard)
  Pride. The name on the door does not take charity. They will force it anyway.
...
- **Pass TARP and inject the cash** `us-tarp-pass` (HISTORY · hard)
  A pause, not a cleanup. The names on the doors stay. The RTC this is not.
- **Stand up an RTC for the houses** `us-rtc-again` (HINDSIGHT · deal)
  ...
  Overlay title: Yes. That was the right thing to do.
  You tried to unwind the houses in daylight, like 1989. Congress hated it. TARP still happens.
...
- **Hold the desk** `ir-hold-2011` (HISTORY · soft)
  The Street continues. The name on the door survived. The warehouse was always empty.
- **Walk** `ir-walk-2011` (MORAL · epilogue · walk)
  Leave the name on the door. A stick figure sits. The machinery grinds on.
  Overlay title: We congratulate you on your moral choice.
...
You are still the desk. The window saved the name on the door. The warehouse was always empty.
...
- **File upstairs** `us-file-upstairs` (HINDSIGHT · hard)
  ...
  Overlay title: Yes. That was the right thing to do.
  You filed upstairs. The dock was not empty. History left it empty.
